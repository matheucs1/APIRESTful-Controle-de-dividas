...
module.exports = class RevenuesController {
    static async registerRevenues(req, res) {
        ...
        if (!typeRevenue) {
            return res.status(422).json({ message: 'O tipo de receita é obrigatorio' });
        }
        ...
        if (!dateEntry) {
            return res.status(422).json({ message: 'A data de entrada é obrigatoria' });
        }
        ...
        try {
            await revenues.save()
            res.status(201).json({ message: "Cadastro da receita realizado com sucesso!" })
        } catch (error) {
    ...
    static async updateRevenues(req, res) {
        ...
        try {
            ...
            res.status(500).send({ menssage: "Erro ao atualizar receita!"})
        }
    }

    static async deleteRevenues(req, res) {
        try {
            const id = req.params.id;
            const deleteRevenues = await Revenues.findByIdAndDelete(id);

            if (deleteRevenues) {
                res.status(200).json({message: "A receita foi excluída com sucesso!"})
            }
        } catch (error) {
